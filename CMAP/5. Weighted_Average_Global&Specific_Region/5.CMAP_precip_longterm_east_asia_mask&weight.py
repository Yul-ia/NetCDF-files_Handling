...
os.chdir('')
### new clim file
ds_precip = xr.open_dataset('CMAP_longterm/CMAP_precip_mon_clim_9120.nc') # [mm/day] > [mm/month]

mask_file = xr.open_dataset('CPC_landsea.nc')  # glb mask for CMAP
asia_mask_file = xr.open_dataset('ASKmaskNew20230615.nc') # asia mask

# Unit : mm/day -> mm/month
for i in range(len(ds_precip.pre_clim)):
    p_time = str(ds_precip.month[i].values)
    b = datetime.datetime.strptime(p_time,'%m')
    nums_of_day = ((b + relativedelta(months=i))-datetime.timedelta(days=1)).day
    ds_precip.pre_clim[i] *= nums_of_day

precip = ds_precip.pre_clim
...
for i in range(length_precip):
    p_time = str(glb_new_grid.month[i].values)
...
new_df = pd.DataFrame({'month':time_lst,'glb_clim':glb_lst, 'es_clim':es_lst,'ys_clim':ys_lst,
                       'ecs_clim':ecs_lst,'ask_clim':ask_lst})

new_df.reset_index(drop=True, inplace=True)
new_df

os.getcwd()
new_df.to_csv('precip_new_lonterm_east_asia_mask&weighted.csv',index=False)
